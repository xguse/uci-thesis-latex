...
Our previous attempts to infer \glspl{CRE} bioinformatically have been mostly successful.
...
This method used no expression information in the process of predicting the CREs.
However we were able to show that a sub-set of the inferred \glspl{CRE} were enriched significantly in abundance profiles derived from a previous affymetrix-based bloodfeeding experiment in \Ag.
Comparing transcriptional abundance profiles between sugarfed and bloodfed (5h PBM) \Aa\ mosquitoes allowed us to predict multiple putative \glspl{CRE} when we focused on transcripts that were detected at high levels in in bloodfed mosquitoes but undetectable in sugarfed females.
...
Possible \glspl{CRM} were again observed by visual inspection in an experiment comparing mRNA abundance profiles between \Aa\ fed with DNV infected blood and those fed with non-infected blood.
In this study certain tissues were dissected and analyzed separately.
This provided a level of tissue specificity as well.
Again, these \gls{CRM} results have yet to be confirmed experimentally.

These studies have also provided useful lessons that I have incorporated into the approach described here.
A common theme among these lessons is the importance of the relative ratio of signal-to-noise contained in the data-set being probed.
For example, in \cite{bonizoni2011}, the most successful \gls{CRE} prediction results arose from a gene-set characterized by fairly a restrictive abundance profile definition.
The requirement that transcripts be highly detected at 5h PBM but completely undetectable in sugarfed mosquitoes yielded a gene-set of just XX members.
\cite{bonizzoni2012complex} further demonstrates that functional genomic results are sensitive to the definition of gene-sets used for analysis.
In addition to abundance profile definition, \cite{bonizoni2011} illustrated that, in \Aa, the official annotation is incomplete and contains at least some level of annotation errors that may be considered particularly troublesome to analyses that rely on accurate definition of putative promoter sequences; however, it also provides support that the current state of annotation is sufficient to allow \gls{CRE} prediction.
Finally, in experiments comparing mRNA abundance profiles between three different strains of a single species(\Aa), \cite{bonizzoni2012strain} illustrates the need to take precautions when comparing differences in transcript abundance profiles between species that may have last shared a common ancestor as far back as 200 mya.
